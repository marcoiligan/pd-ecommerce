...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            profile.profile_picture = 'userprofile/img_avatar.png'
            profile.save()
            ...
            send_email.send()
            #messages.success(request, 'Thank you for registering with us. We have sent you a verification email to activate your account.')
                ...
                if is_cart_item_exists:
                    ...
                    for pv in product_variation:
                        if pv in ex_var_list:
                            item_id = id[ex_var_list.index(pv)]
                            delete_ids.append(v_ids[product_variation.index(pv)])
                            ...
                            for item in cart_item:
                                item.user = user
                                item.save()
                    cart_items_to_delete = CartItem.objects.filter(id__in=delete_ids)
